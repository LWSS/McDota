...
static const char *GameState2String( int state ){
    switch( state ){
        CASE_STRING( DOTA_GAMERULES_STATE_INIT );
        CASE_STRING( DOTA_GAMERULES_WAIT_FOR_PLAYERS_TO_LOAD );
        CASE_STRING( DOTA_GAMERULES_HERO_SELECTION );
        CASE_STRING( DOTA_GAMERULES_STRATEGY_TIME );
        CASE_STRING( DOTA_GAMERULES_PREGAME );
        CASE_STRING( DOTA_GAMERULES_GAME_IN_PROGRESS );
        CASE_STRING( DOTA_GAMERULES_POSTGAME );
        CASE_STRING( DOTA_GAMERULES_DISCONNECT );
        CASE_STRING( DOTA_GAMERULES_TEAM_SHOWCASE );
        CASE_STRING( DOTA_GAMERULES_CUSTOM_GAME_SETUP );
        CASE_STRING( DOTA_GAMERULES_WAIT_FOR_MAP_TO_LOAD );
        default:
