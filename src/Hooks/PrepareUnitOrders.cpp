...
void Hooks::PrepareUnitOrders( CDotaPlayer *thisptr, DotaUnitOrder_t order, int targetIndex, Vector movePosition, int abilityIndex, PlayerOrderIssuer_t orderIssuer,
    ...
    if( mc_command_repeater->GetBool() ){
        ...
        for( int i = 0; i < 30; i++ ){
            localPlayerVMT->GetOriginalMethod<PrepareUnitOrdersFn>(434)( thisptr, order, targetIndex, movePosition, abilityIndex, orderIssuer, entity, queue, queueBehavior, showEffects );
        }
        MC_PRINTF("done.\n");
    }
